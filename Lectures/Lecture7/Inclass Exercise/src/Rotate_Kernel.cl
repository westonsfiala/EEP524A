__constant sampler_t sampler_rotate = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_MIRRORED_REPEAT | CLK_FILTER_LINEAR;
...
)
{
    ...
    // read input image
    float value = read_imagef(inputImg, sampler_rotate, readCoord).x; // return only x component of float4 (monochromatic image)
    // write output image
