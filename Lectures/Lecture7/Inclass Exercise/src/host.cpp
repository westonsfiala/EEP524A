...
static const std::string BASE_DIRECTORY = "C:/work/GitHub/EEP524A/Lectures/Lecture7/Inclass Exercise/";
static const std::string OUTPUT_DIRECTORY = BASE_DIRECTORY + "Outputs/";
static const std::string SRC_DIRECTORY = BASE_DIRECTORY + "src/";
static const std::string BLUR_KERNEL_FILE = SRC_DIRECTORY + "SobelGrad.cl";
...
const static std::string CONV_FILTER_KERNEL_NAME = "SobelGrad";
...
int main(int argc, char** argv)
{
    ...
    for (const auto& lenaFile : LENA_FILES)
    {
        ...
        // Arg 3 - __global float* horizontal_filter

        // Get a vector of all the filter coefs.
        const auto sobelFilters = generateSobelFilters();
        auto horizontalFilter = sobelFilters.first;
        auto verticalFilter = sobelFilters.second;

        auto horizFilterMem = clCreateBuffer(
            chosenContext,
            CL_MEM_USE_HOST_PTR,
            sizeof(float) * horizontalFilter.size(),
            horizontalFilter.data(),
            &success
        );
        ...
        // Arg 4 - __global float* vertical_filter

        auto vertFilterMem = clCreateBuffer(
            chosenContext,
            CL_MEM_USE_HOST_PTR,
            sizeof(float) * verticalFilter.size(),
            verticalFilter.data(),
            &success
        );
        if (!processClCallStatus("clCreateBuffer", success))
        ...
        // Arg 5 - const int filterWidth
        const cl_int filterSize = 3;
        success = clSetKernelArg(
            blurKernel,
            5,
            sizeof(cl_int),
        ...
        const auto runNameBase = "Lena_" + std::to_string(lenaX) + "x" + std::to_string(lenaY) + "_filt_" + std::to_string(filterSize);
        ...
        for (uint32_t i = 0; i < KERNEL_ITERATIONS; ++i)
        {
            ...
            */
            ...
            if (!imagePrinted)
            {
                ...
                auto lenaOutputImageData = malloc(sizeof(uint8_t) * lenaX * lenaY * lenaNumChannels);
